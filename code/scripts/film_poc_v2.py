"""Step B v2 — FiLM speaker-conditioned KWS with P0 trunk init + curriculum.

Improvements over film_poc.py (v1):
  1. Trunk + PCEN initialized from the validated P0 KWS checkpoint.
  2. Curriculum schedule: epochs 1-5 use no imposters (let KWS converge first);
     epochs 6-15 ramp imposter ratio 0 -> 0.5 linearly; epochs 16+ steady at 0.5.
  3. 400 speakers x 50 utts (vs v1's 200 x 30) for more SV diversity.
  4. Lower LR (5e-4) since trunk starts pretrained.

Usage from code/:
    .venv/bin/python scripts/film_poc_v2.py --device mps
    .venv/bin/python scripts/film_poc_v2.py --device cpu
    .venv/bin/python scripts/film_poc_v2.py --smoke --device mps   # quick test
"""
...
OUT_DIR   = REPO_ROOT / "results" / "film_poc_v2"
...
# ---------------------------------------------------------------- dataset
class SCKeywordSpeakerDataset(Dataset):
        ...
        for word in SC_V2_WORDS:
            for wp in (root / word).glob("*.wav"):
                ...
                if spk not in speakers_allowed:
                    continue
                all_items.append((str(wp), WORD_TO_IDX[word], spk))
        per_spk = defaultdict(list)
        for it in all_items:
            per_spk[it[2]].append(it)
        items = []
        for lst in per_spk.values():
            rng.shuffle(lst)
            items.extend(lst[:max_per_speaker])
        rng.shuffle(items)
        self.items = items
    ...
    def __getitem__(self, idx):
        ...
        if len(wav) < CLIP_LEN:
            ...
            wav = wav[:CLIP_LEN]
        return torch.from_numpy(wav), kw, self.speaker_to_idx[spk]
...
def load_p0_init(model: JointKWSSV, ckpt_path: Path) -> int:
    """Transfer frontend + trunk weights from P0 KWS ckpt into the joint model."""
    if not ckpt_path.is_file():
        print(f"  WARN: P0 ckpt not found at {ckpt_path} — using random init")
        return 0
    ...
    print(f"  transferred {len(transferred)} tensors from P0 (frontend + trunk)")
    return len(transferred)


def imposter_ratio_for_epoch(epoch: int, ramp_start: int, ramp_end: int,
                             max_ratio: float = 0.5) -> float:
    """Linear ramp: 0 for epoch <= ramp_start, max_ratio for epoch >= ramp_end."""
    if epoch <= ramp_start:
        return 0.0
    if epoch >= ramp_end:
        return max_ratio
...
def main() -> None:
    ...
    ap.add_argument("--ramp_start", type=int, default=5,
                    help="epoch <= this: imposter_ratio = 0")
    ap.add_argument("--ramp_end", type=int, default=15,
                    help="epoch >= this: imposter_ratio = max_ratio")
    ...
    if args.smoke:
        ...
        args.ramp_start = 2
        args.ramp_end = 4
        print("[smoke mode] 50 train spks, 15 eval spks, 15 utts/spk, 6 epochs")
    ...
    print(f"SC root    : {SC_ROOT}  (exists={SC_ROOT.exists()})")
    print(f"P0 ckpt    : {args.p0_ckpt}  (exists={Path(args.p0_ckpt).is_file()})")
    ...
    # ---- dataset / loader
    train_ds = SCKeywordSpeakerDataset(
    ...
    print(f"model      : {sum(p.numel() for p in model.parameters() if p.requires_grad)/1e3:.1f}K params")
    print(f"P0 init    :")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for wav, kw, spk in train_loader:
            wav = wav.to(device); kw = kw.to(device); spk = spk.to(device)
            B = wav.size(0)
    ...
    torch.save({"model": model.state_dict(), "aam": aam_head.state_dict()},
               OUT_DIR / "joint_kws_sv_film_v2.pt")
    print(f"\nsaved -> {OUT_DIR}/joint_kws_sv_film_v2.pt")
    ...
    summary = {
        "version": "film_poc_v2 (P0 init + curriculum)",
        ...
        "lr": args.lr,
        "model_params_k": round(sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e3, 1),
        "evaluation": {
        ...
        "v1_baseline": {
            ...
            "FAR_percent": 12.50,
            "REJ_percent": 84.38,
            "collapse_ratio": 0.292,
        },
        "delta_vs_v1": {
            "TAR_pp": round(TAR * 100 - 63.75, 2),
            "FAR_pp": round(FAR * 100 - 12.50, 2),
            "REJ_pp": round(REJ * 100 - 84.38, 2),
        },
    }
    json.dump(summary, open(OUT_DIR / "film_summary_v2.json", "w"), indent=2)
    print()
    print(json.dumps(summary, indent=2))

    with open(OUT_DIR / "film_results_v2.csv", "w", newline="") as f:
        w = csv.writer(f)
    ...
    ax1.set_title("FiLM rejection v2 (P0 init + curriculum)", fontweight="bold")
    ...
    ax2.set_title("Keyword-logit distribution v2", fontweight="bold")
    ax2.legend(); ax2.grid(linestyle="--", alpha=0.4); ax2.set_axisbelow(True)
    plt.tight_layout()
    plt.savefig(OUT_DIR / "film_rejection_metrics_v2.png", dpi=200, bbox_inches="tight")
    print(f"wrote {OUT_DIR}/film_rejection_metrics_v2.png")
