"""Step B — FiLM speaker-conditioned KWS rejection POC.

Trains a joint KWS+SV+FiLM model on Speech Commands V2 with explicit imposter
training (50% of batches use mismatched speaker embeddings, target = UNKNOWN).
Evaluates on held-out speakers via enrollment + genuine/imposter trials.

Usage from the code/ directory:
    .venv/bin/python scripts/film_poc.py --smoke              # 2-epoch sanity test
    .venv/bin/python scripts/film_poc.py                      # full run, auto device
    .venv/bin/python scripts/film_poc.py --device cpu         # force CPU
    .venv/bin/python scripts/film_poc.py --device mps         # force Apple GPU
"""
...
# Make `from src.model import ...` work when run from code/
...
OUT_DIR   = REPO_ROOT / "results" / "film_poc"


def parse_speaker(filename: str) -> str:
    """SC V2 filename: <speaker_hash>_nohash_<idx>.wav"""
...
def main() -> None:
    ...
    ap.add_argument("--smoke", action="store_true",
                    help="2 epochs, fewer speakers, quick MPS validation")
    ap.add_argument("--n_train_spk", type=int, default=200)
    ap.add_argument("--n_eval_spk",  type=int, default=50)
    ap.add_argument("--max_per_speaker", type=int, default=30)
    ...
    if args.smoke:
        args.n_train_spk = 30
        args.n_eval_spk = 10
        args.max_per_speaker = 10
        args.epochs = 2
        print("[smoke mode] 30 train spks, 10 eval spks, 10 utts/spk, 2 epochs")
    ...
    print(f"model      : {n_params/1e3:.1f}K params  | "
          f"AAM head (training-only): {sum(p.numel() for p in aam_head.parameters())/1e3:.1f}K")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for wav, kw, spk in train_loader:
            ...
            is_imposter = (torch.rand(B, device=device) < 0.5)
        ...
        imp_rej_rate = imp_rej / max(1, n_imp)
        history.append({
            "epoch": epoch, "loss_kws": lk_sum/n, "loss_sv": ls_sum/n,
            "gen_acc": gen_acc, "imp_rej": imp_rej_rate, "wall_s": dt,
        })
        print(f"ep{epoch:>2} | kws={lk_sum/n:.3f} | sv={ls_sum/n:.3f} "
              f"| gen_acc={gen_acc:.3f} | imp_rej={imp_rej_rate:.3f} | {dt:.1f}s")

    torch.save({"model": model.state_dict(), "aam": aam_head.state_dict()},
               OUT_DIR / "joint_kws_sv_film.pt")
    print(f"\nsaved → {OUT_DIR}/joint_kws_sv_film.pt")

    # ---- eval: enrolment + trials
    ...
    summary = {
        ...
        "evaluation": {
            ...
            "probability_collapse_ratio": round(mean_prob_i / mean_prob_g, 4),
        },
    }
    json.dump(summary, open(OUT_DIR / "film_summary.json", "w"), indent=2)
    print()
    print(json.dumps(summary, indent=2))

    with open(OUT_DIR / "film_results.csv", "w", newline="") as f:
        w = csv.writer(f)
    ...
    ax1.set_title("FiLM rejection — bar metrics", fontweight="bold")
    ...
    ax2.set_title("Keyword-logit distribution", fontweight="bold")
    ax2.legend(); ax2.grid(linestyle="--", alpha=0.4); ax2.set_axisbelow(True)
    plt.tight_layout()
    plt.savefig(OUT_DIR / "film_rejection_metrics.png", dpi=200, bbox_inches="tight")
    print(f"wrote {OUT_DIR}/film_rejection_metrics.png")
