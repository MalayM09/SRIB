...
\begin{frame}{Technical Details — System Architecture}
  ...
  {\scriptsize\color{srigray}
   \textbf{Forward (inference):} waveform $\to$ PCEN frontend $\to$ shared BC-ResNet-8 trunk (blue) $\to$ TRM branching (yellow) $\to$ KWS / SV heads (green) $\to$ \novel{FiLM} (red) modulates KWS logits on the speaker embedding.
   \textbf{Training:} stop-gradient on the SV$\to$FiLM edge --- KWS loss never back-propagates into the speaker branch (otherwise SV degrades; trade-off measured on Slide 11).
   \textbf{Params:} trunk \kpi{324\,K} (validated) + branches/heads $\sim$\kpi{175\,K} $=$ \kpi{499\,K} Phase-1 joint (Slide 13); \kpi{$\sim$830\,K} projected with multi-teacher KD heads (Slide 7). Novel components highlighted red/yellow.}
\end{frame}
...
\begin{frame}{Technical Details — Multi-Teacher Knowledge Distillation}
  \vspace{-0.4em}
  \centering
  \includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{\illus{2_distillation.png}}
  \vspace{-0.2em}
  {\scriptsize
  \[
  ...
  \vspace{0.2em}
  {\scriptsize\color{srigray}
   \textbf{Phase-1 validation:} WavLM-Base+ final-layer distillation (simplest form of $\mathcal{L}_\text{hint}$) trained jointly with $\mathcal{L}_\text{KWS}$ for 40 epochs $\to$ KWS clean val $97.6 \to 98.3\,\%$, cosine alignment with teacher 0.94. Multi-layer hints ($\{4,8,11\}$ as shown above) projected to close the remaining $0.7$\,pp gap to 99\,\% — see Slide 8.}
\end{frame}
...
\begin{frame}{Phase-1 Empirical Validation — KWS Backbone}
  ...
  \begin{columns}[T, onlytextwidth]
    ...
    \column{0.38\textwidth}
      \small
      \begin{block}{\color{sriblue}Path to ${\geq}\,$99\,\% TA Clean — measured trajectory}
        \kpi{97.6\,\%} (P0, 322\,K) $\;\to\;$
        \kpi{98.2\,\%} (P2 wide 705\,K + label-smooth) $\;\to\;$
        \kpi{98.3\,\%} (P3 + WavLM final-layer distill) $\;\to\;$
        \kpi{${\geq}\,$99\,\%} projected (Slide 5 multi-layer SKILL hints + EMA). All within \kpi{${<}\,$24\,\%} of the 3\,M cap.
      \end{block}
      \begin{block}{\color{srired}Dominant robustness lever}
        \textbf{Data diversity, not the front-end.} At worst case (RIR + $-$5\,dB MUSAN): noise-aug training holds \kpi{77\,\%}; clean training collapses to \alert{55\,\%}. \highlight{$+$22\,pp gap}.
      \end{block}
      \begin{block}{\color{sriblue}PCEN vs log-Mel (under noise-aug)}
        Direction stable across 2 seeds at every SNR — PCEN ahead by mean $+$0.41\,pp, with \kpi{${\sim}\,$4$\times$ lower cross-seed variance} at low SNR.
      \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}{Phase-1 Empirical Validation — Custom-Word Generalization}
  ...
  \begin{columns}[T, onlytextwidth]
    ...
    \column{0.38\textwidth}
      ...
      \end{block}
      \begin{block}{\color{srired}Phonetic + cross-lingual generalization confirmed}
        Within English, per-word EER spans 6--18\,\%: words sharing phonemes with the train set (\texttt{forward}, \texttt{follow}) enrol at \kpi{6--7\,\%}; novel combos (\texttt{backward}, \texttt{learn}) at \alert{16--18\,\%}. \textbf{Zero-shot on Tamil} (MSWC, 6 keywords \emph{never seen} at train, $n{=}180$ trials): \kpi{14.7\,\% mean EER} --- only \kpi{$+$2.2\,pp} above English, despite Tamil's Dravidian phonology and the chosen set being mostly minimal pairs (\texttt{indha}/\texttt{andha}/\texttt{vandha}). The model is \emph{phonetically organised}, not language-specific --- precondition for user-chosen triggers in any language.
      \end{block}
